...
- [ ] **Cross-cutting values live in `penny/constants.py` (under `PennyConstants` or a related enum class). Class-specific configuration lives on the class as a class attribute.** The split:
  - **`constants.py`**: values shared by multiple unrelated modules — emojis, thresholds, timeouts, magic numbers, retry counts, well-known protocol-level identifiers, page sizes. Use a `StrEnum` for enumerable sets so consumers reference symbolic names (`ProgressEmoji.THINKING`) rather than raw literals.
  - **Class attribute on the relevant class**: per-agent identity (`HistoryAgent.PREFERENCE_EXTRACTOR_NAME`), per-agent loop caps (`HistoryAgent.PREFERENCE_EXTRACTOR_MAX_STEPS`), per-mode prompt template names. The agent's own configuration belongs on the agent. Tests reading these import from the class, not from `constants.py`.
  - **Forbidden everywhere**: module-level `_PRIVATE_CONSTANT = ...` at the top of `agents/foo.py` or `tools/bar.py` for any value used by more than one call site (including tests). Promote to either a class attribute or `constants.py` based on the rule above. The only exceptions are file-local regex patterns (`_FOO_PATTERN = re.compile(...)`) that exist purely as a compiled-once optimization.
- [ ] **Agent-flow identities (used as cursor keys + write authors) are class attributes on the agent class.** When an `Agent` subclass hosts more than one logical flow, each flow's identity (`PREFERENCE_EXTRACTOR_NAME = "preference-extractor"`) belongs on the class — distinct from `Agent.name` which is the class identity. Do not put these in `PennyConstants` — they're not cross-cutting; they describe *that agent class's* internal flow.
...
### Tool Surface
- [ ] **Every agent gets the full memory tool surface.** Agents call `build_memory_tools(db, embedding_client, agent_name=...)` and install the entire returned list — no hand-curated subsets like `[LogReadNextTool, CollectionWriteTool, DoneTool]`. The system prompt is responsible for telling the model what to do; narrowing the tool list per agent encodes the prompt's intent twice and breaks down the moment a flow needs an unexpected tool (which the new task-agent structure will routinely require). This applies to every memory-driven flow, not just the chat agent.
- [ ] Specialized tools (BrowseTool, email tools, etc.) are still passed by the agent that owns the capability — this rule covers the unified memory surface only.
