...
### Phase 2 — Two-Mode Extraction Pipeline

Add `allow_new_entities` parameter to extraction. Check `search_log.trigger` to determine mode. Add known-only identification prompt. Penny-triggered searches produce facts only.

**Files**: `extraction.py`, `prompts.py`, `tests/agents/test_extraction.py`
**Blocked by**: Phase 1
