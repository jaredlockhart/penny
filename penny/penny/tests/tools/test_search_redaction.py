...
class TestSearchQuotaViaExecute:
    """Tests that execute() handles quota errors gracefully — the public call path.

    These cover the actual code path seen in the production traceback:
    ...
    @pytest.mark.asyncio
    async def test_execute_with_images_quota_error_returns_graceful_result(self):
        """execute() with skip_images=False also handles quota gracefully."""
        tool = _make_quota_tool()
        tool.skip_images = False

        # _search_image returns None by default (no serper key) — won't raise
        result = await tool.execute(query="test query")
        assert isinstance(result, SearchResult)
        assert result.text == PennyResponse.SEARCH_QUOTA_EXCEEDED
        assert result.image_base64 is None
