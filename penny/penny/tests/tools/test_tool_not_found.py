...
class TestFindSimilarTool:
    """_find_similar_tool detects the 'call_' prefix hallucination."""
    ...
    @pytest.mark.asyncio
    async def test_no_did_you_mean_when_no_call_prefix(self):
        """No hint when the called name has no 'call_' prefix match in the registry."""
        registry = ToolRegistry()
        registry.register(StubLookupTool())
        executor = ToolExecutor(registry)
