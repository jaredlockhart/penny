...
@pytest.mark.asyncio
async def test_thinking_context_uses_markdown_headers(
    signal_server, mock_ollama, make_config, _mock_search, test_user_info, running_penny
):
    """Context sections injected into thinking mode use markdown headers."""
    config = make_config(
        inner_monologue_interval=99999.0,
        inner_monologue_max_steps=2,
    )
    ...
    async with running_penny(config) as penny:
        ...
        # Orientation request should have all context sections
        assert len(requests_seen) >= 1
        system_msgs = [m for m in requests_seen[0]["messages"] if m.get("role") == "system"]
        all_system_text = " ".join(m.get("content", "") for m in system_msgs)

        assert "## Recent Conversation" in all_system_text
        assert "## Your Recent Thoughts" in all_system_text
...
async def test_thinking_loop_uses_lean_context(
    ...
    async with running_penny(config) as penny:
        ...
        # Orientation (request 0) should have full context
        orient_msgs = [m for m in requests_seen[0]["messages"] if m.get("role") == "system"]
        orient_text = " ".join(m.get("content", "") for m in orient_msgs)
        assert "## Recent Conversation" in orient_text
        assert "## Your Interests" in orient_text
        assert "## Your Recent Thoughts" in orient_text

        # Agentic loop (request 1) should have lean context
        loop_msgs = [m for m in requests_seen[1]["messages"] if m.get("role") == "system"]
        loop_text = " ".join(m.get("content", "") for m in loop_msgs)
        assert "## Your Plan" in loop_text
        assert "cyberpunk anime" in loop_text
        # Full context sections should NOT be in the loop
        assert "## Recent Conversation" not in loop_text
        assert "## Your Interests" not in loop_text
        assert "## Your Recent Thoughts" not in loop_text
