...
class TestMigrate:
    ...
    def test_0038_fixes_bare_read_recent_in_extraction_prompts(self, tmp_path):
        """Migration 0038 replaces bare read_recent( with log_read_recent(
        while leaving already-correct log_read_recent( occurrences unchanged."""
        import importlib.util
        from pathlib import Path
        ...
        conn.executemany(
            "INSERT INTO memory (name, extraction_prompt) VALUES (?, ?)",
            [
                ("broken", 'read_recent("penny-messages", window_seconds=86400)'),
                ("already_fixed", 'log_read_recent("penny-messages", window_seconds=86400)'),
                ("no_match", "just call done()"),
            ],
        )
        conn.commit()
        conn.close()
        ...
        migration_path = (
            ...
            / "database"
            / "migrations"
            / "0038_fix_read_recent_in_extraction_prompts.py"
        )
        ...
        assert 'log_read_recent("penny-messages"' in rows["broken"]
        assert "read_recent(" not in rows["broken"].replace("log_read_recent(", "")
        assert rows["already_fixed"] == 'log_read_recent("penny-messages", window_seconds=86400)'
        assert rows["no_match"] == "just call done()"
