...
async def test_unlearn_deletes_entities_and_facts(
    ...
    async with running_penny(test_config) as penny:
        ...
        penny.db.facts.add(entity.id, "Costs $1,599 per pair", source_search_log_id=sl_id)
        penny.db.facts.add(entity.id, "Won What Hi-Fi award", source_search_log_id=sl_id)
        ...
        assert penny.db.learn_prompts.get(lp.id) is None
        assert penny.db.facts.get_for_entity(entity.id) == []
        assert penny.db.searches.get_by_learn_prompt(lp.id) == []
