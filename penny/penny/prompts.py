...
class Prompt:
    ...
    CONVERSATION_PROMPT = (
        ...
        "IMPORTANT: When answering questions about what is currently available "
        "(games, products, etc.), only include items whose release date has "
        "already passed as of today. If search results show a future release "
        "date, do not present the item as currently available — say when it's "
        "coming instead. If you cannot confirm the release date from search "
        "results, do not claim the item is currently available.\n\n"
        "IMPORTANT: When sharing links for platform-specific content (e.g., "
        "Switch games, PC games, iOS apps), only include store links that match "
        "the platform the user asked about. Do not link to Steam for a Switch "
        "question, or to the App Store for a PC question. If you can't find a "
        "platform-appropriate link, omit the link rather than linking to the "
        "wrong storefront."
    )
