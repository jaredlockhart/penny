...
class ChatAgent(Agent):
    ...
    async def _send_proactive(self, user: str) -> bool:
        """Check-in if eligible, otherwise generate candidates and send the best."""
    ...
    async def _send_best_candidate(self, user: str) -> bool:
        """Generate N candidates (1 news + N-1 thought), score, send the best."""
        n = int(self.config.runtime.PROACTIVE_CANDIDATES)
        candidates = await self._generate_candidates(user, n)
        if not candidates:
    ...
    async def _generate_candidates(self, user: str, n: int) -> list[ProactiveCandidate]:
        """Generate 1 news + 1 followup + top-N thought candidates."""
        candidates: list[ProactiveCandidate] = []

        # Fetch thoughts first — top thought anchors entity context for news/followup candidates
        thoughts = await self._get_top_thoughts(user, n)
        anchor = thoughts[0].content if thoughts else None

        # 1x news
        news = await self._generate_one_candidate(
            user, Prompt.PROACTIVE_NEWS, thought=None, entity_anchor=anchor
        )
        if news:
            logger.info("Candidate news: %s", news.answer[:60])
            candidates.append(news)

        # 1x followup on recent conversation
        followup = await self._generate_one_candidate(
            user, Prompt.PROACTIVE_FOLLOWUP, thought=None, entity_anchor=anchor
        )
        if followup:
            logger.info("Candidate followup: %s", followup.answer[:60])
    ...
    async def _generate_one_candidate(
        self, user: str, prompt: str, thought: Thought | None, entity_anchor: str | None = None
    ) -> ProactiveCandidate | None:
        """Generate a single proactive candidate via the agentic loop."""
        self._proactive_thought = thought
        anchor = thought.content if thought else entity_anchor
